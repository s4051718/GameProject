...
public class GameCombat {
    ...
    public static void combatMethod(GameCharacters player, GameCharacters enemy) {
        ...
        while (fightOn) {
            ...
            try {
                ...
                int potionCount = items.Inventory.getHealthPotionCount();
                if (potionCount > 0) {
                    System.out.printf("(3). Use Potion (%d left)\n", potionCount);
                }                
                int choice = scanner.nextInt();
                switch (choice) {
                    case 1:
                        System.out.println("-------------Combat-------------");
                        System.out.printf("\nYou attempt to hit %s", enemy.getName());
                        ...
                        if (enemy.getCurrentHealth() > 0) {
                            System.out.printf("\nThe %s attempts to hit you", enemy.getName());
                            if (enemy.getLevel() == 1) {
                            ...
                            System.out.printf("\nYou have %d health remaining", player.getCurrentHealth());
                            System.out.printf("\nThe %s has %d heatlh remaining", enemy.getName(), enemy.getCurrentHealth());
                            System.out.println("\n--------------------------------");
                        }
                        break;
                    case 2:
                        ...
                        break;
                    case 3:
                        if (items.Inventory.getHealthPotionCount() > 0) {       
                            items.Inventory.useHealthPotion();          
                        } else {                                               
                        System.out.printf("\nThe %s attempts to hit you\n", enemy.getName());
                            if (enemy.getCurrentHealth() > 0) {
                                System.out.printf("\nThe %s attempts to hit you\n", enemy.getName());
                ...
                if (enemy.getCurrentHealth() <= 0) {
                    ...
                    System.out.printf("\nYou have %d\\%d health remaining",Game.pCharacter.getCurrentHealth(),Game.pCharacter.getBaseHealth());              
                    break;
                }       
            } catch (Exception e) {
                int index = items.Inventory.findItemIndex("Health Potion");
                if (index != -1) {
    ...
    public static void levelUp(GameCharacters player, GameCharacters enemy) {
        ...
        if (player.getExp() >= 100 && player.getLevel() < 5) {
        ...
        } else if (player.getExp() >= 35 && player.getLevel() < 2) {
            player.setLevel(2);
            player.setBaseHealth(player.getBaseHealth() + 10);  
            System.out.println("You gained a level!");
            System.out.println("Your base health went up!");            
            ...
            System.out.printf("\nYou have %.0f experience!", player.getExp());
        }
    }
    ...
    public static void lootEnemy(GameCharacters player, GameCharacters enemy) {
            ...
            if (enemy.getName().equals("Bandit")) {
                if (diceRoll.nextBoolean()) {
                    System.out.println("The enemy dropped a Health Potion!");
                    items.Inventory.addHealthPotions(1);
                }
            } else if (enemy.getName().equals("Dryad")) {
                if (diceRoll.nextBoolean()) {
                    System.out.println("The enemy dropped a Health Potion!");
                    items.Inventory.addHealthPotions(1);
                }                
            } else if (enemy.getName().equals("Shreik")) {
                System.out.println("The enemy dropped a Health Potion!");
                items.Inventory.addHealthPotions(2);
            } else if (enemy.getName().equals("Orc") || enemy.getName().equals("Goblin") || enemy.getName().equals("Wight") || enemy.getName().equals("Wraith"))  {
                System.out.println("The enemy dropped a Health Potion!");
                items.Inventory.addHealthPotions(2);                
            } else if (enemy.getName().equals("Balrog") || enemy.getName().equals("Frost Dragon"))  {
               System.out.println("The enemy dropped a Health Potion!");
                items.Inventory.addHealthPotions(4);                
            }
        }
}